...
mod tests {
    ...
    #[test]
    fn test_op_00e0() {
        let mut cpu = instantiate_cpu(vec![0x00e0]);
        cpu.vram = [[128; CHIP8_WIDTH]; CHIP8_HEIGHT];
        cpu.step(delta_time: f32, memory: &mut Memory, keypad: &mut Keypad, display: &mut Display, speaker: &mut Speaker, debug_cpu: bool, debug_memory: bool)();
